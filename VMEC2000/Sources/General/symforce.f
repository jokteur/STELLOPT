...
#ifdef _ANIMEC
     2  , ppar_0, pperp_0,
     3    sigma_an0 , tau_an0 , pbprim_0, ppprim_0, densit_0
#elif defined _FLOW
     4  ,  prot_0 , protrsq_0 , prprim_0
#endif
C-----------------------------------------------
...
#elif defined _FLOW
            prota(jk,i)     = p5*(prot(jk,i)       -prot(jka,ir))
            prot_0(jk)      = p5*(prot(jk,i)       +prot(jka,ir))
            protrsqa(jk,i)  = p5*(protrsq(jk,i)    -protrsq(jka,ir))
            protrsq_0(jk)   = p5*(protrsq(jk,i)    +protrsq(jka,ir))
            prprima(jk,i)   = p5*(prprim(jk,i)     -prprim(jka,ir))
            prprim_0(jk)    = p5*(prprim(jk,i)     +prprim(jka,ir))
#endif
