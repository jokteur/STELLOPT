...
!
!$omp parallel
!$omp+ private(m,mj,j2,jl,n,ni,k,l,jll,j2l,jmaxl,nsl,i,mparity)
!$omp+ private(work1,tempr,tempz)
...
!
     ...
     2                           + work1(jll:nsl,10)*cosnvn(k,n)
            END DO
         END DO
      END DO                   ! Loop m  for OpenMP parallelisation 
!$omp end do
...
!     f0 is the m=0 component of frho, f2 is m=2 component.
      ...
      ALLOCATE (work1(ns,1),    stat=i)
      IF (i .ne. 0) STOP 'Allocation error at end of VMEC2000 tomnsps'
         DO jl = 2, ns-1
